...
|------------------------|---------------------------------------|
| A deep learning model can distinguish between healthy and infected cherry leaves with at least 90% accuracy.| The **Performance on Test Set** table on Model Performance page show the model predicting with a 99.76% degree of accuracy. This hypothesis can be considered validated.|
| Resizing images to a smaller fixed resolution (e.g., 50×50) retains enough visual detail for accurate classification. |The original dataset images are 256x256 pixels. By using smaller input dimensions (160x160) Git LFS was avoided while the model was still able to maintain an accuracy of 99.76%. This hypothesis can be considered validated. |
| Average and variability images for each class (healthy vs. infected) will show distinct patterns.|The **Average and Variability Image Differences** and **Average Healthy & Infected Cherry Leaves Differences** images on the Visual Studies page show the differences in the form ofvisible white spot/streak patterns. Average images also reveal a color difference where infected leaves appear as a lighter shade of green compared to healthy ones. This hypothesis can be considered validated.|
| Using an ML model will reduce manual inspection time by over 90% per tree. | The mildew detector application allows for a near instantaneous prediction of the presence of mildew on a cherry leaf vs the problem statement of 30 minutes per tree for inspection. This hypothesis can be considered validated. |
